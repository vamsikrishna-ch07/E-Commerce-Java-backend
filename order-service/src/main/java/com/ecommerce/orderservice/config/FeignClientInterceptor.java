...
public class FeignClientInterceptor implements RequestInterceptor {
    ...
    public void apply(RequestTemplate template) {
        ...
        // The name "order-service-client" should match what's in your application.yml or centralized config.
        ClientRegistration clientRegistration = clientRegistrationRepository.findByRegistrationId("order-service-client");
        ...
        OAuth2AuthorizeRequest authorizeRequest = OAuth2AuthorizeRequest.withClientRegistrationId(clientRegistration.getRegistrationId())
                .principal("order-service") // The principal name can be anything for client_credentials
                .build();
